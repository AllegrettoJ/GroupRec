...
def tourrecommendationloop(startNode, endNode, budget, dfNodes, subtourCons, userInterest, intWeight, normInterest, startNodeVT):
    ...
    ind = 0
    nodeRevisit = pd.DataFrame(np.zeros(shape =(len(dfNodes['from'].unique()) + len(dfNodes['to'].unique()), len(dfNodes.index)), dtype = int))
    ...
    if normInterest == True: # whether to normalize user interest or use the raw values
        userInterest.catIntLevel = userInterest.catIntLevel / max(userInterest.catIntLevel)  # normalize user interests to [0,1]
    ...
    for i in nodeRevisit.index:
        model += pulp.lpSum(visiting_status[is_visit] * nodeRevisit.iloc[i][is_visit] for is_visit in dfNodes.index) <= 1


    # constraint path is connective
    for i in nodeConnect.index:
        model += pulp.lpSum(visiting_status[is_visit] * nodeConnect.iloc[i][is_visit] for is_visit in dfNodes.index) == 0
    ...
    # break subtours by setting constraint of RHS having <= subtourEdgeCount-1
        ...
        if (rhsValue < 0):
            rhsValue = 0
        model += pulp.lpSum(visiting_status[is_visit] * noSubtours.iloc[i][is_visit] for is_visit in dfNodes.index) <= rhsValue
    ...
    if pulp.LpStatus[model.status] == 'Optimal':
        ...
        while True:
            ...
            newSubtourCons.iloc[ind, list(dfNodes[(dfNodes['from'] == prevTempNode) & (dfNodes['to'] == tempNode)].index)] = 1 # new subtour constraint
            if tempNode == endNode:
                break
        print(dfTemp)
        print(dfSoln)
        ...
        if len(dfTemp.index) != 0:
            while len(dfTemp.index) != 0:
                ...
                newSubtourCons.iloc[ind, list(dfNodes[(dfNodes['from'] == subtourNode) & (dfNodes['to'] == tempNode)].index)] = 1
